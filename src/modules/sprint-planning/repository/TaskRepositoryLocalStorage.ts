...
export default class TaskRepositoryLocalStorage implements TaskRepository {
  ...
  get data$() {
    return this._data.asObservable().pipe(map(items => items.map(item => TaskDTO.toDomain(item))));
  }

  private load() {
    this._data.next(JSON.parse(localStorage.getItem(StorageKey.DATA_TASK_PLANNING) ?? "[]"));
  }

  private save() {
    localStorage.setItem(StorageKey.DATA_TASK_PLANNING, JSON.stringify(this.data ?? []));
  }
